import { InferAttributes, FindOptions, Attributes } from "sequelize";
...
class Service {
   ...
   * @returns
   */
  async handleAddCategory(data: Attributes<InstanceType<typeof Category>>): Promise<InstanceType<typeof Category> | Error> {
   ...
   * @returns
   */
  async handleCategoryListing(body: any): Promise<InstanceType<typeof Category>[]> {
    const searchableFiled = ["title"];
    let [query, limit, offset]: [FindOptions<InstanceType<typeof Category>>, number, number] = CommonService.generateListingQuery(body, searchableFiled);
    const categories:InstanceType<typeof Category>[] = await new CategoryRepository().getListingData(Category, query, { limit, offset });
  ...
  async handleUpdateCategory(body: any): Promise<number | Error> {
    ...
    const category:InstanceType<typeof Category> | null = await new CategoryRepository().checkAlreadyExist(Category, query);
    if (!category) {
   ...
   * @returns
   */
  async handleGetCategories(body: any): Promise<InstanceType<typeof Category>[]> {
    const searchableField = ["title"];
    let query: FindOptions<InstanceType<typeof Category>> = {}
    query.where = CommonService.generateSearchText(searchableField, body.searchText);
    const categories: InstanceType<typeof Category>[] = await new CategoryRepository().getListingData(Category, query, {});
    if (_.isEmpty(categories)) {
      return [];
    }
    return categories;
